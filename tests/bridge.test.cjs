...
function addon() {
  const calls = [];
  const tabs = [
    { id: 5, windowId: 2, pinned: true, title: "Custom title", url: "https://example.com" },
  ...
  const context = vm.createContext({
    ...
    browser: {
      tabs: {
        ...
        query: async (query) => tabs.filter((tab) => query.pinned === undefined || tab.pinned === query.pinned),
  ...
  vm.runInContext(fs.readFileSync(path.join(__dirname, "../addon/background.js"), "utf8"), context);
  return { request: (value) => context.handleRequest(value), calls, tabs };
}
...
test("lists nonprivate pins and activates the exact existing ID without navigation", async () => {
  ...
  await request({ method: "activate", session: tabs[0].session, tabId: tabs[0].tabId });
...
test("rejects stale sessions and private tabs without activation", async () => {
  const { request, calls } = addon();
  await assert.rejects(request({ method: "activate", session: "old", tabId: 5 }), /expired/);
  await assert.rejects(request({ method: "activate", session: "session-1", tabId: 6 }), /not available/);
  assert.equal(calls.length, 0);
});

test("native host round trip over private socket and fragmented native frames", { timeout: 10000 }, async () => {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "zr-"));
  const child = spawn(process.execPath, [path.join(__dirname, "../native/host.cjs")], {
  ...
  try {
    ...
    const body = Buffer.from(JSON.stringify({ id: request.id, result: [{ tabId: 5 }] }));
...
test("pinned titles survive loading while ordinary page titles stay live", async () => {
  const { request, tabs } = addon();
  await request({ method: "list" });
  tabs[0].title = "Server page title";
  tabs[2].title = "New ordinary title";
  const results = await request({ method: "list" });
  assert.equal(results[0].title, "Custom title");
  assert.equal(results[0].pageTitle, "Server page title");
  assert.equal(results[1].title, "New ordinary title");
});
