...
if (process.platform !== "darwin")
  throw new Error("This installer currently supports macOS only.");
const directory = path.join(os.homedir(), ".zen-browser-bridge");
fs.mkdirSync(directory, { recursive: true, mode: 0o700 });
fs.copyFileSync(
  path.join(__dirname, "native/host.cjs"),
  path.join(directory, "host.cjs"),
);
fs.copyFileSync(
  path.join(__dirname, "native/titles.cjs"),
  path.join(directory, "titles.cjs"),
);
...
console.log(
  "Native host installed. Load addon/manifest.json in Zen about:debugging for development.",
);
